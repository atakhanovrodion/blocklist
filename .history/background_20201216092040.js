...
chrome.storage.local.get("blocked", function (items) {
  if (items.blocked) blockedSites = items.blocked;
});
...
function unlistSite(tabid, site) {
  ...
  if (i > -1) blockedSites.splice(i, 1);
  ...
  if (request && request.url) {
    if (request.type == "main_frame") {
      ...
      chrome.tabs.getSelected(null, function (tab) {
        ...
        console.log(
          "tab blocking state set for tab " + tab.id + " to " + tabBlockingState
        );
      });
...
function updateMapping(details) {
  ...
  console.log(
    "replacing tab " + details.replacedTabId + " with tab " + details.tabId
  );
